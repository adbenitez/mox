...
type Domain struct {
	...
	Chatmail                    bool             `sconf:"optional" sconf-doc:"Enable chatmail mode for this domain. When enabled: 1. Accounts are automatically created on first login attempt via IMAP or SMTP submission with valid credentials (username matches email format at this domain). 2. Only encrypted connections are allowed (TLS/STARTTLS required). 3. Spam filtering and graylisting are disabled for fast delivery. This is intended for use with Delta Chat and similar instant messaging over email clients. Incoming SMTP must still have RequireSTARTTLS enabled on the listener, and submission/IMAP must have NoRequireSTARTTLS set to false."`
